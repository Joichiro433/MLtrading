    ...
    while True:
        if datetime.now().minute % 15 == 0:
            ...
            if now_position.side == constants.NONE:
                if pred_buy > 0:
                    order = Order(side=constants.BUY, order_type=constants.LIMIT, qty=qty, price=buy_price)
                    api_client.create_order(order=order)
                if pred_sell > 0:
                    order = Order(side=constants.SELL, order_type=constants.LIMIT, qty=qty, price=sell_price)
                    api_client.create_order(order=order)
            # exit
            elif now_position.side == constants.BUY:
                if not (pred_sell < 0 and pred_buy > 0):  # ML予測が、「価格がまだ上がる」場合は保留
                    order = Order(side=constants.SELL, order_type=constants.LIMIT, qty=now_position.size, price=sell_price)
                    api_client.create_order(order=order)
            elif now_position == constants.SELL:
                if not (pred_buy < 0 and pred_sell > 0):  # ML予測が、「価格がまだ下がる」場合は保留
                    order = Order(side=constants.BUY, order_type=constants.LIMIT, qty=now_position.size, price=buy_price)
                    api_client.create_order(order=order)
